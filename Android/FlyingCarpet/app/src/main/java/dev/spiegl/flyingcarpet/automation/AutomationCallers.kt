...
object AutomationCallers {
    ...
    private fun signingSha256(context: Context, pkg: String): String? = runCatching {
        ...
        // `signingInfo` and GET_SIGNING_CERTIFICATES are API 28. This app's minSdk is 29, so the
        // second arm is unreachable *here* — it is kept because this file is copied verbatim across
        // the family and sister apps with minSdk 26 need it: on a 26–27 device the flag is accepted
        // and `signingInfo` comes back null, so without the branch the door would refuse every
        // caller. The deprecated array is the correct answer there, not a compromise: before key
        // rotation existed, `signatures` WAS the signing certificate.
