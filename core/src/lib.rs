    ...
    pub fn snapshot(&self, current_done: u64, current_size: u64) -> (u8, String) {
        ...
        match self.total_bytes {
            ...
            _ => {
                ...
                let percent = ((completed + within) / self.num_files.max(1) as f64 * 100.0).round() as u8;
...
pub async fn start_transfer<T: UI>(
    ...
    if using_bluetooth {
        match negotiate_bluetooth(&mode, ble_ui_rx, &interface, ui).await {
            Ok((p, _ssid, pw)) => {
                peer = Some(p);
                if password.is_none() {
                    password = Some(pw);
    ...
    match mode {
        Mode::Send(files) => {
            ...
            for (i, file) in files.iter().enumerate() {
                ...
                match sending::send_file(file, common_folder, &key, &mut stream, &mut totals, ui).await {
                    Ok(_) => (),
                    Err(e) => {
        ...
        Mode::Receive(folder) => {
            ...
            for i in 0..num_files {
                ...
                match receiving::receive_file(&folder, &key, &mut stream, &mut totals, ui, last_file).await {
                    Ok(_) => (),
                    Err(e) => {
